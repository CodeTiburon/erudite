    ...
    function loadGames()
    {
        ...
        $.get(url, {}, function(list) {
            ...
            $(list).each(function(index, item) {
                var countries = jQuery.parseJSON( item.flags );
                var countriesStr = '';
                $(countries).each(function(index, item) {
                    countriesStr += '<i class="'+item.toLowerCase()+' flag"></i> ';
                });
                
                var topics = item.topics;
                var topicsStr = '';
                $(topics).each(function(index, item) {
                    topicsStr += item.title+', ';
                });
                    ...
                    $.get(url, {}, function(game) {
                        ...
                        $(game.players).each(function(index, item) {
                            $('#game-board-players').append(
                                            '<div class="item">'
                                            +'<img class="ui avatar image" src="http://www.gravatar.com/avatar/'+item.image+'.jpg?d=monsterid&s=70">'
                                            +'<div class="content">'
                        ...
                        $('#view-game-page div.twelve').hide();
                        $('#game-board-join').show();
                    });
                });
            });
        });
